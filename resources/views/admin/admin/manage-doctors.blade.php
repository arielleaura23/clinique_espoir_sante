                        ...
                        <tbody>
                            @forelse($doctors as $index => $doctor)
                                <tr>
                                    ...
                                    <td>{{ $doctor->specialization ?? 'Non défini' }}</td>
                                    <td>{{ $doctor->name }}</td>
                                    <td>{{ $doctor->email }}</td>
                                    <td>{{ $doctor->phone ?? '—' }}</td>
                                    <td>{{ $doctor->consultancy_fees ? number_format($doctor->consultancy_fees, 0, ',', ' ') . ' FCFA' : '—' }}
                                    </td>
                                    <td>
                                        {{ $doctor->CreationDate
                                            ? \Carbon\Carbon::parse($doctor->CreationDate)->format('d/m/Y H:i')
                                            : $doctor->created_at->format('d/m/Y H:i') }}
                                    </td>
                                    <td>
                                        <div class="visible-md visible-lg hidden-sm hidden-xs">
                                            <a href="{{ route('admin.doctor.edit', $doctor->id) }}"
                                                class="btn btn-transparent btn-xs" title="Edit">
