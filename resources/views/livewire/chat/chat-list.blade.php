<div x-data="{ type: 'all', query: '{{ $query }}' }" x-init="$nextTick(() => {
    ...
    <header class="px-3 z-10 bg-white sticky top-0 w-full py-2">
        ...
        <div class="flex gap-3 items-center overflow-x-scroll p-2 bg-white" style="justify-content: space-between;">
            <div>
            ...
            <div>
                <a href="{{route('users')}}" class="inline-flex justify-center items-center rounded-full gap-x-1 text-xs font-medium px-3 lg:px-5 py-1  lg:py-2.5 border " >
                    <img src="{{asset('assets/img/edit.png')}}" alt="new chat" class="w-4 h-4">
                </a>
            </div>
                ...
                @foreach ($conversations as $key => $conversation)
                    <li id="conversation-{{ $conversation->id }}" wire:key="{{ $conversation->id }}"
                        ...
                        <a href="#" class="shrink-0">
                            <x-avatar :user="$conversation->getReceiver()" />
                        </a>
